...
class Person():
        ...
        for i in range(period): #period
            ...
            for i in OtherRooms:
                while(self.start < i[1]):
                    day.append([str(self.start),i[0],1]) #TODO: Randomise 1 (Not Sleeping) somehow 
                    self.start = self.start + datetime.timedelta(minutes=5)
    ...
    def generateDay(self):
        WakeupTime = self.start + datetime.timedelta(hours=7) + datetime.timedelta(minutes=random.randint(0, 60))
        BackFromWork = WakeupTime + datetime.timedelta(hours=8) + datetime.timedelta(minutes=random.randint(0, 60))

        
        #Start Activity = BackFromWork + Margin time (Max 18)
        SleepTime = self.start + datetime.timedelta(hours=23) + datetime.timedelta(minutes=random.randint(0, 59))
